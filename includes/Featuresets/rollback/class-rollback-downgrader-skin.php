...
class Rollback_Downgrader_Skin extends WP_Upgrader_Skin {
	...
	public function header() {
		...
		echo '<div class="wrap">';
		echo '<h1>' . esc_html__( 'Rolling back plugin…', 'really-simple-featured-audio' ) . '</h1>';
		echo '<div class="rsfa-rollback-log">';
	}
	...
	public function after() {
		...
		if ( is_wp_error( $this->result ) || ! $this->result ) {
			echo '<div class="notice notice-error">';
			echo '<p>' . esc_html__( 'Rollback failed during installation.', 'really-simple-featured-audio' ) . '</p>';
			...
			echo '<p>' . esc_html__( 'Rollback complete. The plugin files have been restored.', 'really-simple-featured-audio' ) . '</p>';
			echo '</div>';
		}

		echo '<p><a class="button button-primary" href="' . esc_url( admin_url( 'plugins.php' ) ) . '">';
		echo esc_html__( 'Go to Plugins page', 'really-simple-featured-audio' );
		echo '</a></p>';
	}
}
